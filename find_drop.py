...
def main(args):
    ...
    for TW_id in pf.TW_ids:
        ...
        try:
            ...
            #max_date, min_date = get_max_min_date(df, "2020-01-21", "2020-05-12")
            max_date, min_date = "2020-05-12", "2020-01-21" 
            ...
            change = get_change(df, start_date, end_date)
    ...
    for k in sorted_changes[:100]:
        ...
        TW_ids.append(TW_share_list[TW_share_list.code == k].name.values[0])
        TW_id_change_values.append(changes[k])
        #counter[] += 1
    ...
    plt.figure()
    plt.xticks(fontproperties=myfont, rotation=90)
    plt.bar(TW_ids, TW_id_change_values)
    plt.show()
